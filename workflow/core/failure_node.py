...
def flyte_failure_node_wf(name: str = "my_cluster"):
    ...
    subwf3(name="another_cluster")  # -> a failure task node and a failure task node inside the subsubworkflow
    subwf4(name="another_cluster")  # -> a failure workflow node (subwf3). a failure node inside a failure node
    subwf5(name="another_cluster")  # -> a failure task node and this failure node will fail
    t = t1(a=1, b="2")
    d = delete_cluster(name=name)
    c >> t >> d
